...
## Phase 12 — 적극적 되묻기 (UX/요구사항 상세화) (2026-08-09)

**배경:** planner가 "정보가 크게 부족할 때만" 되묻도록 보수적이라 대부분 추측해서 바로 생성했다. UI/UX 정의를 더 명확·상세하게 뽑도록 **첫 설계 전 적극적으로, 다양한 축으로 되묻게** 함.

### 변경 — `lib/engine/groqEngine.ts`

- **되묻기 규칙 재작성**: 첫 요청이 조금이라도 모호하면 되묻기(사용자·목적·데이터·유형·검색/필터·상태·권한·예외·연계 9개 축 중 의미 있는 것 폭넓게). 질문은 **서로 다른 축**을 다루도록 명시. 3~~6개, 보기 2~~5개.
- **coerceQuestions 상한 상향**: 질문 4→6개, 보기 4→5개.
- **무한 질문 방지(코드 가드)**: `planDesign`에서 대화 이력에 `[질문함]`이 있거나 `currentHtml`이 있으면 questions를 무시하고 design/edit로 진행 — **되묻기 최대 1라운드**. 프롬프트 규칙에만 의존하지 않고 코드로 강제.
- **edit 힌트 보강**: 수정·개선 대화(currentHtml 존재)에선 되묻지 말고 바로 edit/design.

### 검증 (dev, 실키)

- A. 모호("조합원 관리 화면") → **5개 다축 질문**(사용자/핵심작업/데이터항목/레이아웃/검색·필터, multiSelect 적절) ✓
- B. 상세 프롬프트(컬럼·검색·유형 명시) → 질문 없이 바로 `design` ✓
- C. 수정 대화(currentHtml + "상태 필터 추가") → 질문 안 하고 진행 ✓
- D. 되물은 이력 있음(2번째 턴) → 코드 가드로 질문 반복 차단, `design` ✓
- `typecheck` / `lint` / `build` 통과 ✓

---

## Phase 13 — 비주얼 리디자인 (Claude Design 임포트) (2026-08-09)

**배경:** Claude Design 프로젝트(`DesignMate UI.dc.html`)의 새 디자인 언어를 앱에 반영. 기능은 그대로, 시각만 전면 재스타일. 핵심 원칙: NH 그린은 액션·강조에만, 배경은 따뜻한 오프화이트, 헤더는 얇게, 여백은 넓게, 좌측 고정 레일이 프로젝트 목록을 대신.

### 변경 — `app/layout.tsx`, `app/globals.css`, `app/page.tsx`, `components/AuthGate.tsx`

- **폰트**: Pretendard(본문) + Gowun Dodum(디스플레이 제목) CDN 로드. `--font`/`--font-display` 토큰.
- **디자인 토큰 전면 교체**(`:root`): 그린 `#00873c`→`#2D8A3E`, 페이지 배경 `#EDECE5`, 표면 `#FAFAF5`/`#F6F5EF`, 레일 `#F2F1EA`, 라인 `#E4E4DC`, ink 계열(#1A1F14~#9AA391), danger `#E5564A`. 토큰이 앱 전체로 캐스케이드되며 대부분 자동 재스타일.
- **로그인(1a)**: `AuthGate`를 좌측 브랜드 패널(Gowun 히어로 "필요한 화면을…") + 우측 폼 분할로 재구성. 모바일에선 브랜드 패널 숨김.
- **좌측 레일(사이드바)**: 기존 드로어 → **데스크톱 상시 264px 레일**(NH로고·새 대화·최근 프로젝트·사용자 푸터). ≤900px에서 슬라이드인 드로어 + 슬림 모바일 상단바(☰/타이틀/＋)로 전환. 앱 셸을 `column`→`[rail | main]` 가로 플렉스로 변경.
- **패널**: 얇은 56px 헤더, 결과 pane 웜 알트 표면(`#F6F5EF`), 버튼/입력 라운드·색 갱신. 죽은 `.drawer*`·`.user-chip` CSS 제거.
- **참고**: Claude Design 캔버스 런타임(`_ds_bundle.js`/`support.js`)은 뷰어 전용이라 앱엔 미반영. 디자인 의도만 구현.

### 검증 (prod 빌드 + 헤드리스 크롬)

- 로그인 1a: 분할 브랜드/폼, Gowun 히어로, 그린 액션 스크린샷 확인 ✓
- 빈 상태 1b(데스크톱): 264px 레일 + 온보딩 3단계 + 예시 칩 + 결과 pane ✓
- 빈 상태 1b(모바일 390px): 슬림 상단바 + 대화/결과 탭 + 온보딩 ✓
- 결과 1d: 활성 프로젝트 하이라이트, 정의서(Gowun 제목·태그·컴포넌트 표) + 데스크톱/모바일 토글·와이어프레임 ✓
- `lint`(0 warning) / `typecheck` / `build` 모두 통과 ✓

---

## Phase 14 — 대시보드(e) · 내보내기 모달(f) · 모바일 결과(g) (2026-08-09)

**배경:** 리디자인(Phase 13)의 나머지 화면 e/f/g를 추가. **기존 기능은 손대지 않고** 시각/화면만 추가(순수 additive).

### 변경 — `app/page.tsx`, `app/globals.css`

- **f. 내보내기 모달**: 결과 툴바의 ↓버튼 2개를 그린 "내보내기" 버튼 1개로 대체(복사 버튼은 유지). 클릭 시 모달 — UI/UX 정의서(.md) / 와이어프레임 HTML 옵션 + 파일명 미리보기. **각 옵션은 기존 `download()`를 그대로 호출**(로직 무변경). `exportBase()`로 안전 파일명 생성.
- **e. 프로젝트 대시보드**: 레일 "최근 프로젝트" 옆 "전체 보기" → `dashboardOpen` 오버레이. "내 프로젝트" 제목·통계(전체/이번 주)·카드 그리드(3열). 카드 클릭=**기존 `switchProject()`**, ✕=**기존 `deleteProject()`**, "새 화면 만들기"=**기존 `newProject()`**. 데이터는 기존 `projects` 재사용.
- **g. 모바일 결과 탭**: 기존 `mobileView` 토글 그대로. 새 토큰으로 자동 폴리시(슬림 상단바·세그먼트 탭·결과 정의서/와이어프레임), 대시보드·모달도 모바일 반응형.

### 검증 (prod 빌드 + 헤드리스 크롬)

- 기존 기능 무손상: 테스트 중 프로젝트 2개 생성·서버 저장·버전 표시 정상 → 대시보드에 2개 카드로 확인 ✓
- e 대시보드: "내 프로젝트"·통계(2/2)·카드 3개(프로젝트2+새화면) 스크린샷 ✓
